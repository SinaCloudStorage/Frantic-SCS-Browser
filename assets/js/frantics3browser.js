...
var FranticS3Browser = function () {
    ...
    var generate_bucket_listing = function (files) {
        ...
        for (i=0; i < files.length; i++) {
            ...
            var signedurl = '/' + encodeURIComponent(name).replace('%2F', '/');// + '?' + jQuery.param(signedparamsdata);
            var signature = sign_api(expires, signedurl);

            var paramsdata = {'KID': ('sina,' + aws_access_key_id), 'Signature': signature, 'Expires': expires};
    ...
    var init_bucketlist = function () {
        ...
        jQuery(function() {
                $.ajax({
                        ...
                        data: {'KID': ('sina,' + aws_access_key_id), 'Signature': signature, 'Expires': expires},
                        dataFormat: 'xml',
                        cache: false,
